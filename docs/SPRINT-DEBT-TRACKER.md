...
SD-001 | 2026-07-07 | CRITICAL | .env file committed and tracked in git; all contained credentials treated as compromised. FIXED by FIX-MSM-008: operator rotated all keys (old values dead) BEFORE the fix; .env added to .gitignore (.env, .env.*, !env.example), git rm --cached .env (file remains on disk, untracked — git ls-files empty, check-ignore matches); env.example rewritten with the 7 client-safe variable names. RESIDUAL RISK: the old .env blobs remain in git history (reachable from main/dev); purging requires a history rewrite + force-push that the main ruleset blocks — accepted as residual since every historical value is rotated/dead. Revisit only if the repo goes public. | B0.3 → FIX-MSM-008 | RESOLVED 2026-07-08 (residual: dead secrets in history)

SD-008 | 2026-07-07 | CRITICAL | Service-role key stored as VITE_SUPABASE_SERVICE_ROLE_KEY in tracked .env (client env namespace AND git). FIXED by FIX-MSM-008: key rotated by operator; variable dropped entirely from env.example with an explicit comment (service-role key lives only in Supabase edge function secrets, never in this repo/client env); repo-wide scan confirms NO source file references VITE_SUPABASE_SERVICE_ROLE_KEY (only .env itself, env.example placeholder pre-fix, and historical docs). Operator must also delete the variable from their local .env; nothing in code reads it. Note: the inert browser-side VITE_OPENAI_API_KEY pattern in src/services/originality/embeddings.ts remains tracked under SD-018 (dead/mock surfaces). | INV-MSM-000 §6 → FIX-MSM-008 | RESOLVED 2026-07-08 (residual: dead value in history)
